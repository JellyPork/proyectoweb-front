<template>
  <v-container>
    <v-data-table :headers="headers" :items="arrendatarios" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat>
...
export default {
  ...
  methods: {
        ...
        .then((response) => {
          ...
          this.arrendatarios.forEach((arrendatario) => {
            ...
            const propiedad = this.propiedades.find(
              (propiedad) => propiedad.id === arrendatario.propiedadId
            );

            // Si se encuentra el propietario, asignar su personaId a la propiedad propietario
            if (arrendatario) {
              arrendatario.propiedad = propiedad.cve_catastral;
            }
            //i++;
          });
        })
    ...
    editItem(item) {
      this.editedIndex = this.arrendatarios.indexOf(item);
    ...
    deleteItemConfirm() {
      const personaId = this.arrendatarios[this.editedIndex].id;
      const token = localStorage.getItem("token");
      axios
        .delete(`http://localhost:3000/arrendatarios/${personaId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        .then(() => {
          // Remove the item from the arrendatarios array
          this.arrendatarios.splice(this.editedIndex, 1);
          this.closeDelete();
        })
        .catch((error) => {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios
          .put(
            `http://localhost:3000/arrendatarios/${personaId}`,
            this.editedItem,
            {
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          .then((response) => {
            // Update the item in the arrendatarios array
            this.arrendatarios.splice(this.editedIndex, 1, response.data);
            this.close();
          })
          .catch((error) => {
