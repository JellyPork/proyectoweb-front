<template>
  <v-container>
    <v-card class="signupContainer">
      <v-card-title>Agregar Arrendatario</v-card-title>
      ...
      <v-card-actions>
        <!-- Sign up button -->
        <v-btn @click="addArrendatario" color="primary">Add Arrendatario</v-btn>
      </v-card-actions>
    </v-card>
  </v-container>
</template>
...
const addArrendatario = () => {
  ...
  axios
    .post(
      "http://localhost:3000/arrendatarios/",
      {
        personaId: formData.value.persona_id,
        propiedadId: formData.value.propiedad_id,
      },
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    .catch((error) => {
      console.error("Error adding arrendatario:", error);
    });
};
